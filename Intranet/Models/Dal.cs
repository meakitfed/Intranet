            ...
            try
            {
				...
				foreach (NoteDeFrais n in nathan.NotesDeFrais)
				{
					for (int j = 0; j < 5; j++)
					{
						...
						AjoutLigneDeFrais(nathan.Id, n.Id, new LigneDeFrais { Nom = labelsLigne[rand], Complete = true, Mission = Missions[rand2], Somme = rand * rand2 * 5, Statut = (n.Actif ? StatutLigneDeFrais.EnAttente : StatutLigneDeFrais.Validée), Date = DateTime.Now });
					}
				}
